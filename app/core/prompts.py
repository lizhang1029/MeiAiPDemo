...
def build_scoring_prompt(
    ...
    *,
    ...
    parts.append("\n# 输出要求")
    item_schema = (
        '[{"item_key":"...","item_name":"...","max_score":数字,"score":数字,'
        '"deductions":[{"reason":"扣分原因","points":扣分值,"evidence":"引用证据原文"}],'
        '"rationale":"评分依据"}]'
        if dimension.items
    ...
    schema = {
        ...
        "level": "优秀/良好/合格/不合格",
        "items": item_schema,
        "deductions": '[{"reason":"扣分原因","points":扣分值,"evidence":"引用证据"}]',
        "rationale": "本维度总体评分依据",
