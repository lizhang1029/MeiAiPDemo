...
### 2.4 整段面试转写与按题分段（`/transcribe_full`）

实际考务中，一名考生整场面试是**一个连续的音/视频文件**，按题目顺序作答，每题作答前有「等待读题」的明显停顿。系统提供 `POST /transcribe_full` 入口，把整段转写后按停顿切分为与题目顺序对应的多段回答：

```mermaid
graph LR
    A[上传整段面试音/视频] --> B[ffmpeg 抽音轨/重采样 16k wav]
    B --> C[Paraformer 带句级时间戳转写]
    C --> D[按相邻句停顿 ≥ pause_ms 切分]
    D --> E["segments[] 按题目顺序"]
    E --> F[各段在评分阶段经 2.2 清洗后逐题打分]
```

要点：
1. **分段依据**：Paraformer 返回句级时间戳（`begin_time`/`end_time`）；相邻句间隔 `≥ pause_ms`（默认 2000ms，前端可调）视为「题前停顿」，开启新一题。
2. **只做时间分段，不做说话人分离**：当前不区分考官/考生（Paraformer 同步识别不返回 diarization）；考官读题、「可以开始作答/时间到/下一题」等内容保留在各段内，由 2.2 清洗在评分阶段剔除。
3. **题目对齐**：导入试题 JSON 仅提供**题干**；各段按顺序与各题题干、各维度对齐。维度名/满分/评分标准以**手动录入的评分表**为准，**总分以评分表满分合计为准**。
4. **降级策略**：无 Key/缺依赖/无有效停顿时返回内置示例整段转写（普通话 5 题，含停顿、考官读题、考务口令），demo 离线可演示分段与清洗。实现见 `app/core/asr.py` 的 `transcribe_segments()` 与 `_segment_by_pause()`。
